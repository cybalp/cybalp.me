...
// ❯ UTILITY FUNCTIONS
// ❯ @doc Formats date to MM-DD format.
...
// ❯ @doc Formats tag list with hash prefix.
function formatTag(tagList: string[]) {
	return tagList.map((t) => `#${t}`).join(" ");
}

// ❯ @doc Checks if post category matches filter targets.
...
// ❯ COMPUTED DATA
// ❯ @doc Filters and groups posts by year with applied filters.
let groups = $derived.by(() => {
	...
	if (categories.length > 0) {
		filteredPosts = filteredPosts.filter((post) =>
			isCategoryMatch(post.data.category, categories),
		);
	}

	if (uncategorized !== null) {
		filteredPosts = filteredPosts.filter(
			(post) => !getCategoryPathLabel(post.data.category),
		);
	}
