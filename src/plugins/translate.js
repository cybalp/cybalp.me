...
var translate = {
	...
	config: {
		...
		get: () => {
			...
			data.language.autoDiscriminateLocalLanguage =
				translate.autoDiscriminateLocalLanguage;
			data.language.range = translate.language.translateLanguagesRange;
			data.language.urlParamControl.use =
				translate.language.setUrlParamControl_use;
			data.language.urlParamControl.name =
				translate.language.setUrlParamControl_name;
			...
			data.ignore.class.conditionFunction =
			...
			data.request.listener.delayExecuteTime =
				translate.request.listener.delayExecuteTime;
			data.request.listener.minIntervalTime =
			...
			data.visual.webPageLoadTranslateBeforeHiddenText.use =
				translate.visual.webPageLoadTranslateBeforeHiddenText_use;
		...
		set: (data) => {
			...
			if (
				typeof data.language.select.languages === "string" &&
				data.language.select.languages.trim().length > 0
			) {
				translate.selectLanguageTag.languages = data.language.select.languages;
			}
			if (
				typeof data.language.local === "string" &&
				data.language.local.trim().length > 0
			) {
				translate.language.setLocal(data.language.local);
			}
			if (
				typeof data.language.defaultTo === "string" &&
				data.language.defaultTo.trim().length > 0
			) {
				translate.language.setDefaultTo(data.language.defaultTo);
			}
			if (
				typeof data.language.autoDiscriminateLocalLanguage === "boolean" &&
				data.language.autoDiscriminateLocalLanguage === true
			) {
				translate.setAutoDiscriminateLocalLanguage();
			}
			if (
				typeof data.language.range === "object" &&
				data.language.range.length > 0
			) {
			...
			if (
				typeof data.language.urlParamControl.name === "string" &&
				data.language.urlParamControl.name.trim().toLowerCase() !== "language"
			) {
			...
			if (
				typeof data.selectionTranslate.use === "boolean" &&
				data.selectionTranslate.use === true
			) {
				if (translate.selectionTranslate.use === false) {
			...
			if (
				typeof data.progress.api.use === "boolean" &&
				data.progress.api.use === true
			) {
				if (translate.progress.api.use === false) {
			...
			if (typeof data.network.use === "boolean" && data.network.use === true) {
				if (translate.network.isUse === false) {
					translate.network.use();
				}
			}
			if (
				typeof data.visual.webPageLoadTranslateBeforeHiddenText.use ===
					"boolean" &&
				data.visual.webPageLoadTranslateBeforeHiddenText.use === true
			) {
				if (
					translate.visual.webPageLoadTranslateBeforeHiddenText_use === false
				) {
	...
	selectLanguageTag: {
		...
		changeLanguageBeforeLoadOfflineFile: (path) => {},
		...
		customLanguagesHandle: (languageList) => {
	...
	changeLanguage: (languageName) => {
		...
		translate.reset({
			selectLanguageRefreshRender: false,
		});
			...
			for (let i = 0; i < iframes.length; i++) {
				...
				try {
					...
					if (typeof iframe.src === "string") {
						translate.log(
							"change sub page iframe exception , iframe src : " + iframe.src,
						);
					}
					translate.log(e);
				}
	...
	ignore: {
		...
		class: {
			...
			push: (className, conditionFunction) => {
				if (translate.ignore.class.data.indexOf(className) > -1) {
					translate.log(
						"translate.ignore.class.push 设置异常，所设置的 className: " +
							className +
							" 已存在里面了，所以此次设置被放弃",
					);
					return;
				...
				if (typeof conditionFunction !== "function") {
					translate.log(
						"translate.ignore.class.push 设置异常，所设置的第二个参数类型不是function，被抛弃",
					);
					return;
				}
				...
				if (
					...
					for (var c_index = 0; c_index < classNames.length; c_index++) {
						if (
							...
							if (
								translate.ignore.class.data.indexOf(classNames[c_index]) > -1
							) {
								if (
									...
									return translate.ignore.class.conditionFunction[
										classNames[c_index]
									](ele);
								}
								return true;
    ...
    */
	...
	nomenclature: {
        ...
        */
         ...
         */
		replace: (text, nomenclatureKey, nomenclatureValue, nodeObject) => {
			/*
            ...
            */
			...
			if (wordKeyIndex > -1) {
				...
				var texts = []; //它是text经过pos的分割后的数组，要返回的数组
         ...
         */
		dispose: (textArray, nomenclatureKey, nomenclatureValue, nodeObject) => {
			// 输入验证
	...
	offline: {
        ...
        */
			...
			for (var uuid in translate.nodeQueue) {
				...
				for (var lang in translate.nodeQueue[uuid].list) {
					...
					for (var hash in translate.nodeQueue[uuid].list[lang]) {
						...
						var result = translate.storage.get(
		...
		showPanel: () => {
			...
			textdiv.innerHTML =
				'1. 首先将当前语种切换为你要翻译的语种<br/>2. 点击导出按钮，将翻译的配置信息导出<br/>3. 将导出的配置信息粘贴到代码中，即可完成<br/><a href="http://translate.zvo.cn/4076.html" target="_black" style="color: aliceblue; text-decoration: underline;">点此进行查阅详细使用说明</a>';
        ...
        */
		append: (to, properties) => {
			...
			for (
				var line_index = 0;
				line_index < line.length && line_index < 10;
				line_index++
			) {
				...
				if (maxKey !== "0") {
					if (line[line_index].startsWith(maxKey)) {
						...
						if (line[line_index].trim().length > 0) {
							translate.log(
								"WAINING : translate.offline.append 异常，发现某行的配置项缩进异常，这行的缩进应该跟其他行的缩进保持一致！异常的这行配置项为：\n" +
									item,
							);
						}
					}
			...
			export: async (to) => {
				...
				for (var i in data) {
					...
					if (
						...
						typeof data[i].value[to] === "undefined" ||
						data[i].value[to] === null ||
						data[i].value[to].trim().length === 0
					) {
						continue;
					}
    ...
    */
	...
	listener: {
			...
			start: () => {
				...
				Object.defineProperty(HTMLInputElement.prototype, "value", {
					...
					set(newValue) {
						...
						// 执行原生赋值
						translate.listener.input.originalValueDescriptor.set.call(
							this,
							newValue,
						);
						...
						if (newValue !== oldValue) {
							...
							if (translate.node.find(this)) {
								...
								if (
									typeof nodeData.lastTranslateRenderTime === "number" &&
									Date.now() - nodeData.lastTranslateRenderTime < 100
								) {
        ...
        */
		nodeValueChangeNeedTranslate: (node) => {
			...
			if (translate.node.get(node) != null) {
				...
				if (
					typeof translate.node.get(node).translateResults !== "undefined" &&
					typeof translate.node.get(node).translateResults[node.nodeValue] ===
						"number"
				) {
					...
					if (
						...
						if (
							typeof translate.node.get(node).lastTranslateRenderTime ===
								"number" &&
							translate.node.get(node).lastTranslateRenderTime + 30 > Date.now()
						) {
		...
		addListener: () => {
					...
					if (mutation.type === "childList") {
						if (mutation.addedNodes.length > 0) {
							...
							for (var ani = 0; ani < mutation.addedNodes.length; ani++) {
								var addNodeName = translate.element
									.getNodeName(mutation.addedNodes[ani])
									.toLowerCase();
								if (addNodeName === "iframe") {
									//如果是iframe，还要进行注入进去翻译
								...
								if (
									addNodeName.length > 0 &&
									translate.ignore.tag.indexOf(addNodeName) == -1
								) {
			...
			if (
				typeof translate.element.tagAttribute["input"] === "object" &&
				translate.element.tagAttribute["input"].attribute.indexOf("value") > -1
			) {
	...
	renderTask: class {
		...
		execute() {
			...
			for (const node of this.taskQueue.keys()) {
				var tasks = this.taskQueue.get(node);
				if (tasks == null) {
					continue;
				}
			...
			for (var hash in this.nodes) {
				...
				for (
					...
					for (var task_index = 0; task_index < tasks.length; task_index++) {
						...
						if (
							typeof translate.node.data.get(translateNode) === "undefined" ||
							translate.node.data.get(translateNode) === null
						) {
							translate.log(
								"执行异常，渲染时，node 未在 translate.node 中找到, 这个很有可能是点击过快，上一个翻译任务还在网络请求中，又点击了别的地方导致内容又被改变。当前异常已被容错。 node：",
							);
							translate.log(translateNode);

							var getNodeText = translate.element.nodeAnalyse.get(
								node,
								task["attribute"],
							);
							...
							translate.node.setModified(
								translateNode,
								"create:translate.renderTask.execute",
							);
						}
						...
						var analyseSet = translate.element.nodeAnalyse.set(
							...
							task.resultText,
							task["attribute"],
							typeof task["participles"] === "undefined"
								? []
								: task["participles"],
						);
    ...
    */
	waitingExecute: {
        ...
        */
		isAllExecuteFinish: (uuid, from, to, result, info) => {
	...
	lifecycle: {
		...
		changeLanguage: {
            ...
            */
			trigger: [],
			trigger_Trigger: (data) => {
				for (
					var i = 0;
					i < translate.lifecycle.changeLanguage.trigger.length;
					i++
				) {
					try {
				...
				for (
					var i = 0;
					i < translate.lifecycle.changeLanguage.old_trigger_array.length;
					i++
				) {
					try {
            ...
            */
			...
			push: (func) => {
				translate.log(
					"提示， translate.lifecycle.changeLanguage.push 方式已过时，但依旧生效，可正常使用。 最新的方式，文档参考 http://translate.zvo.cn/540189.html ",
				);
            ...
            */
			resetAfter: [],
			resetAfter_Trigger: (data) => {
				var isNextExecute = true; //是否继续向下执行，true则是继续执行，false则是不继续执行。
				for (
					var i = 0;
					i < translate.lifecycle.changeLanguage.resetAfter.length;
					i++
				) {
					try {
        ...
        */
		execute: {
            ...
            */
			...
			translateNetworkBefore_Trigger: (data) => {
				/*
            ...
            */
			...
			translateNetworkAfter_Trigger: (data) => {
				for (
					var i = 0;
	...
	execute: (docs) => {
        ...
        */
		...
		if (all.length > 1500) {
			translate.log("------tip------");
			translate.log(
				"警告 translate.execute( docs ) 传入的docs.length 过大，超过1500，这不正常，当前 docs.length : " +
					all.length +
					" , 它依旧会正常进行执行，但会有性能损耗。 这个情况很可能是你用的某些框架，没有等dom渲染完就执行了translate.execute() ，同时启用了dom变动监听，导致监听到页面加载大量的DOM渲染； 另外也有可能你本身页面就是列表页或者什么原因就是有大量的动态元素获取后渲染，如果是这种情况，这是正常的。",
			);
		}
			...
			for (var hash in translate.nodeQueue[uuid]["list"][lang]) {
				...
				if (cache != null && cache.length > 0) {
					for (
						...
						var translateNodeData = translate.node.get(
							translate.nodeQueue[uuid]["list"][lang][hash]["nodes"][node_index]
								.node,
						);
						var participles = [];
						if (
							typeof translateNodeData !== "undefined" &&
							translateNodeData !== null &&
							typeof translateNodeData.whole === "boolean" &&
							translateNodeData.whole === false
						) {
							...
							for (var translateText_original in translateNodeData.translateTexts) {
								...
								if (
									!Object.hasOwn(
										translateNodeData.translateTexts,
										translateText_original,
									)
								) {
									continue;
								}
                ...
                */
				...
				for (
					...
					translateTextNodeMap
						.get(lang)
						.set(
							translate.nodeQueue[uuid]["list"][lang][hash].nodes[ni].node,
							1,
						);
				}
        ...
        */
		...
		for (var lang_index in fanyiLangs) {
			...
			for (const key of translateTextNodeMap.get(lang).keys()) {
	...
	node: {
        ...
        */
         ...
         */
		get: (node) => translate.node.data.get(node),
		//判断某个node (key) 是否在 translate.node.data 中是否存在，如果存在返回true
		find: (node) => translate.node.get(node) != null,
		set: (node, value) => {
        ...
        */
		setModified: (node, text) => {
			if (
				typeof translate.node.data.get(node) === "undefined" ||
				translate.node.data.get(node) === null
			) {
				translate.log(
					"translate.node.setModified exception: node not find in translate.node,  node:",
				);
				translate.log(node);
				return;
			}
        ...
        */
		refresh: () => {
			...
			for (const key of translate.node.data.keys()) {
				// 检查节点是否还在DOM中
	...
	element: {
        ...
        */
		...
		nodeAnalyse: {
            ...
            */
			set: (node, originalText, resultText, attribute, participles) =>
			...
			/*
                ...
                返回结果是一个数组，其中：
                    ...
                    node: 要进行翻译的目标node
                            注意，如果是对 input、textarea 的value进行翻译，而 value是通过js赋予的，那么这个value属性的值并不是一个单独的node，所以是为空的，此时要记录的node便是  input、textarea 这个node本身。
            */
			analyse: (node, originalText, resultText, attribute, participles) => {
				var result = []; //返回的结果
				result["node"] = node;
				result["text"] = "";
            ...
            */
			analyseReplaceBefore_DateToTranslateNode: (
				node,
				attribute,
				resultShowText,
			) => {
		...
		iframe: {
			...
			use: (translateJsUrl) => {
				if (
					...
					console.log(
						"ERROR: 您当前设置了 translate.element.iframe.use(...); 但是您当前的协议是file协议访问的，这个协议访问会存在跨域问题，所以您的设置没有生效。 您可以通过本地开一个访问服务，以 http、https 等正常请求的方式进行访问。",
					);
					return;
				}
            ...
            */
			...
			isIframeSameOrigin: (iframe) => {
				...
				try {
					...
					iframeUrl =
						iframe.contentDocument?.URL || iframe.contentWindow?.location.href;
				} catch (e) {
					// 跨域时：取src，若src为空则默认identity:blank
					iframeUrl = iframe.src || "identity:blank";
				}

				// 4. 特殊处理：identity:blank 视为未跨域
				if (iframeUrl === "identity:blank") return true;
			...
			iframeJsInjector: class {
				...
				importJsAndTranslateExecute = function () {
					...
					try {
						...
						// 用 iframe 新文档创建 script（此时是新文档，不是之前的 identity:blank）
						...
						script.onload = () => {
							...
							iframeContentWindow.translate.to =
								iframeContentWindow.translate.language.getCurrent();
					...
					if (typeof this.iframe === "object") {
						...
						if (typeof this.iframe.contentDocument === "object") {
							...
							if (typeof this.iframe.contentDocument.readyState === "string") {
								...
								console.log(
									"iframe - " + this.translateJsUrl + " state is not string",
								);
							}
						} else {
							console.log(
						...
						console.log(
							"iframe - " + this.translateJsUrl + " is not find (not object)",
						);
					}
				};
			},
            ...
            */
			execute: (iframeTag) => {
				...
				if (
					translate.element.iframe.iframeMap.get(iframeTag) === null ||
					typeof translate.element.iframe.iframeMap.get(iframeTag) ===
						"undefined"
				) {
				...
				if (
					typeof iframeTag.src === "string" &&
					iframeTag.src.trim().length > 0
				) {
					//是通过 src 加载内容的
					...
					if (
						typeof translate.element.iframe.iframeMap.get(iframeTag).addLoad !==
							"boolean" ||
						...
						iframeTag.addEventListener("load", () => {
							...
							if (
								...
								translate.element.iframe.iframeMap.get(iframeTag).isTranslate =
									true;
								var ifr = new translate.element.iframe.iframeJsInjector(
									iframeTag,
									translate.element.iframe.translateJsUrl,
								);
								ifr.injectJs();
							}
						});
        ...
        */
		...
		findNode: (uuid, node) => {
			...
			if (node.nodeType === 2) {
				//是属性node，比如 div 的 title 属性的 node
	...
	addNodeToQueue: (uuid, node, text, attribute) => {
        ...
        */
		...
		if (addQueueObjectArray.length == 0) {
			...
			//将翻译完成后要显示出的文本进行记录
			translate.node.get(translateNode).resultText =
				translate.element.nodeAnalyse.get(node, attribute).text; //直接获取当前node显示出来的文本作为最后的结果的文本
    ...
    */
	...
	language: {
        ...
        */
		generateLanguageNameObject: () => {
			var languages = new Map();
			for (var key in translate.language.systems) {
				if (!Object.hasOwn(translate.language.systems, key)) {
					continue;
				}
				for (
					var li = 0;
        ...
        */
		recognition_languageName_force: (recognition_result) => {
			...
			if (
				...
				translate.language.translateLocal === false
			) {
			...
			if (
				...
				for (var language in recognition_result.languageArray) {
					...
					if (!Object.hasOwn(recognition_result.languageArray, language)) {
						continue;
					}
	...
	util: {
        ...
        */
		text_full_half_width_generate: (text, language) => {
			//console.log(text);
			...
			if (requireSpace) {
				...
				if (punctuationMarks_fullWidth.indexOf(findTextFirstChar) > -1) {
					...
					if (punctuationMarks_fullWidth.indexOf(findTextLastChar) > -1) {
						originalArray.push(
							processFirstCharText.substring(
								0,
								processFirstCharText.length - 1,
							) +
								punctuationMarks_halfWidth[
				...
				if (punctuationMarks_halfWidth.indexOf(findTextFirstChar) > -1) {
					...
					if (processLastCharText.charAt(1) === " ") {
						originalArray.push(
							processLastCharText.substring(0, 1) +
								processLastCharText.substring(2, findText.length),
						);
					}
					...
					if (punctuationMarks_halfWidth.indexOf(findTextLastChar) > -1) {
						originalArray.push(
							processLastCharText.substring(0, processLastCharText.length - 1) +
								punctuationMarks_fullWidth[
						...
						if (processLastCharText.charAt(1) === " ") {
							originalArray.push(
								processLastCharText.substring(0, 1) +
									processLastCharText.substring(
										2,
										processLastCharText.length - 1,
									) +
									punctuationMarks_fullWidth[
										punctuationMarks_halfWidth.indexOf(findTextLastChar)
									],
		...
		textReplace: (
			...
			if (typeof participles === "object") {
				for (var pi = participles.length; pi >= 0; pi--) {
					var participlesItemArray =
						translate.util.text_full_half_width_generate(
							participles[pi],
							language,
						);
            ...
            */
			 ...
			 * 根据不同的中英文，标点符号后面是否跟空格也不同
			 */
			var originalArray = translate.util.text_full_half_width_generate(
				translateOriginal,
				language,
			);
			/*
            ...
            */
			...
			for (var i = indexArray.length - 1; i > -1; i--) {
				...
				if (translate.language.wordBlankConnector(language)) {
					...
					const originalIndex = indexArray[i];
					...
					if (afterCharIndex < text.length) {
						const char = text.charAt(afterCharIndex);
        ...
        */
		findParticiple: (text, findText, translateTexts) => {
			var resultArray = [];
			...
			// 【方案1】使用下标数组记录位置
			const flagArr = translate.util.participleIndexFind(
				text,
				newTranslateTexts,
			);
		 ...
		 */
		participleIndexFind: (text, newTranslateTexts) => {
			...
			for (let i = newTranslateTexts.length - 1; i >= 0; i--) {
				// 取出分词
				const word = newTranslateTexts[i];
				// 找出分词在原文中出现的所有位置
        ...
        */
		...
		/*
            ...
            ["你好123世界", "你好世界4", true],
        */
		compareStringsIgnoringNumbers: (a, b) => {
			let i = 0,
				j = 0;
			const lenA = a.length,
				lenB = b.length;
        ...
        */
		...
		edge: {
			api: {
				//edge浏览器的翻译功能
				translate:
					"https://edge.microsoft.com/translate/translatetext?from={from}&to={to}&isEnterpriseClient=false", //翻译接口
			},
			language: {
				json: [
	...
	request: {
        ...
        */
		translateText: (obj, func, abnormalFunc) => {
			var texts = [];
				...
				setInterval(() => {
					...
					if (
						...
						if (translate.executeTriggerNumber > 0) {
							//已经执行过了 translate.execute() ，那么才会触发
							try {
            ...
            */
			startUITip: (config) => {
				...
				if (translate.progress.api.isTip) {
					...
					translate.lifecycle.execute.translateNetworkBefore.push((data) => {
					...
					translate.lifecycle.execute.translateNetworkAfter.push((data) => {
						//取出当前变动的node，对应的元素
						var elements = translate.element.nodeToElement(data.nodes);
	...
	js: {
        ...
        */
		transString: (
			...
			try {
				...
				// ❯ @doc Removed eval() for security. Using safe JSON parsing only.
				translate.log("Failed to parse JSON:", e);
				failureFunction("Invalid JSON format");
				return;
			}
        ...
        */
		transObject: (
			...
			const tj_find = translate.js.find(jsObject);
			const kvs = tj_find.stringResult;
			...
			translate.request.translateText(obj, (data) => {
				...
				if (Object.keys(tj_find.functionResult).length > 0) {
					for (var sf in tj_find.functionResult) {
						if (!Object.hasOwn(tj_find.functionResult, sf)) {
							continue;
						}
        ...
        */
		find: (obj, parentKey = "") => {
			const kvs = {}; //stringResult
			const frs = {}; //functionResult
        ...
        */
		objToString: (obj, formatSupplementaryCharLength) => {
				...
				for (var li = 0; li < lines.length; li++) {
					...
					if (lines[li].includes("___TRANSLATEJS_LINE_BREAK___")) {
						...
						if (valueMatch && valueMatch[1]) {
							// 替换换行标记为实际换行
							const functionStr = valueMatch[1].replace(
								/___TRANSLATEJS_LINE_BREAK___/g,
								"\n",
							);
    ...
    */
	visual: {
		 ...
		 */
		filterRectsByLineInterval: (rects, line) => {
			// 验证输入
        ...
        */
		rectsSpaceEliminateOverlap: (inputRects) => {
				...
				if (node.nodeType === Node.TEXT_NODE) {
					...
					if (
						...
						if (translate.node.get(node) !== null) {
							if (typeof translate.node.get(node).resultText !== "string") {
								...
								translate.node.get(node).resultText =
									translate.node.get(node).originalText;
							}
							translate.node.get(node).resultText =
								translate.node.get(node).resultText + "\u00A0";
					...
					if (lastChild && lastChild.nodeType === Node.TEXT_NODE) {
						...
						if (
							...
							lastChild.textContent.substring(
							...
							if (translate.node.get(lastChild) !== null) {
								if (
									typeof translate.node.get(lastChild).resultText === "string"
								) {
									translate.node.get(lastChild).resultText =
										translate.node.get(lastChild).resultText + "\u00A0";
									translate.node.get(lastChild).translateResults[
										translate.node.get(node).resultText
									] = 1;
									translate.node.get(lastChild).lastTranslateRenderTime =
										Date.now();
								}
							}
        ...
        */
         ...
         */
		webPageLoadTranslateBeforeHiddenText: (config) => {
			if (typeof config == "undefined") {
				config = {};
			}
                ...
                */
				...
				translate.lifecycle.execute.translateNetworkBefore.push((data) => {
					...
					if (
						...
						if (
							typeof translate.visual.hideText.first_translate_request_number ==
							"undefined"
						) {
				...
				translate.lifecycle.execute.renderFinish.push((uuid, to) => {
			...
			translate.lifecycle.execute.finally.push((data) => {
		...
		execute: {
            ...
            */
			start: () => {
				...
				//翻译开始
				translate.lifecycle.execute.start.push((uuid, to) => {
                ...
                */
				...
				translate.lifecycle.execute.renderFinish.push((uuid, to) => {
					...
					var translateTexts = []; //翻译的文本的数组，翻译的原文本
					var translateLanguages = []; //翻译的语种数组
		...
		documentCreateTextNode: {
            ...
            */
			use: () => {
				...
				//当用户点击切换语言时触发
				translate.lifecycle.changeLanguage.push((to) => {
				...
				//当第一次打开页面执行翻译时，触发
				translate.lifecycle.execute.start.push((data) => {
    ...
    */
	recycle: () => {
		...
		for (var uuid in translate.time.execute.data) {
			if (!Object.hasOwn(translate.time.execute.data, uuid)) {
				continue;
			}
	...
	init: () => {
		...
		translate.lifecycle.execute.start.push((uuid, to) => {
			...
			for (var currentLanguage in translate.nomenclature.data) {
				if (!Object.hasOwn(translate.nomenclature.data, currentLanguage)) {
					continue;
				}
